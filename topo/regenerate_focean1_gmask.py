...
with netCDF4.Dataset(incname) as ncin:
    ncvar = ncin.variables[ivname]
...
gindices_land = gindices[focean1m1[gindices] == 0]   # Only mask out land areas
focean1m1[gindices_land] = 2
...
# ------------------------------------------------------
# Generate the PNG file (from the NetCDF file)

focean1m = None
with netCDF4.Dataset(oncname) as ncin:
    vname = 'FOCEAN'
    ncvar = ncin.variables[ivname]
    focean1m = np.zeros(ncvar.shape, dtype=np.uint8)
    focean1m[:] = ncvar[:]
...
palette[0,:] = (0,255,0)        # FOCEAN=0 = Land: Green
palette[1,:] = (0,0,255)        # 1 = Ocean: Blue
palette[2,:] = (255,255,255)    # 2 = Greenland: White
